...
const Auth = (() => {
  ...
  const getAccount  = () => { const s = localStorage.getItem(K.ACCOUNT); return s ? JSON.parse(s) : null; };

  const updateBalance = (amount) => {
    const bal  = parseFloat(amount).toFixed(2);
    const curr = getCurrency();
    localStorage.setItem(K.BALANCE, bal);
    document.querySelectorAll('[data-balance]').forEach(el => { el.textContent = `${curr} ${bal}`; });
  };
  ...
  const autoLogin = () => {
    ...
    DerivAPI.on('error', (err) => {
      if (err.type === 'authorize') { clearSession(); window.location.href = 'index.html'; }
    });
  };

  const logout = () => {
    clearSession(); DerivAPI.disconnect();
